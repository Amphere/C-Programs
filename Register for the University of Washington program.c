//this program registers a student for the University of Washington in detail
//Any one who reads this may enjoy !
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdbool.h>
...
course * makeCourse()
{
    ...
    getString("Enter the name of the course: ", aCourse->name);
    return aCourse;
}
...
{
    university * aUniversity = (university *) malloc(sizeof(university));
    aUniversity->numberOfCourses = 0;
    aUniversity->numberOfStudents = 0;
    getString("Enter the name for university: ", aUniversity->name);
    return aUniversity;
}
...
course * getCourse(university * uni)
{
    ...
    getString("Enter the name for the course: ", courseName);
...
{
    student * stu = getStudent(uni);
    course * cour = getCourse(uni);
    if(stu && cour)
    {
        stu->registeredCourses[stu->numberOfRegisteredCourses]=cour;
        stu->numberOfRegisteredCourses++;
        printf("Registered %s for %s course\n", stu->name, cour->name);
    }
    else
    {
        printf("error: student or course not found!");
    }
...
bool isStudentInCourse(student * aStudent, char * courseName)
{
    ...
    {
        if(strcmp(aStudent -> registeredCourses[i]->name, courseName) == 0)
        {
            return true;
        }
    }
...
void listAllStudentsInCourse(university * uni)
{
    ...
    if(cour)
    {
        printf("Students enrolled in course %s are: \n", cour->name);
        for(int i= 0 ; i<uni->numberOfStudents; i++ )
        {
            student * temStudent = uni -> students[i];
            if(isStudentInCourse(temStudent, cour->name))
            {
                printf("%s\n", temStudent->name );
            }
        }
    ...
    {
        printf("error: cannot find course\n");
    }
